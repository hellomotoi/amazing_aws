<template>
  <div id="activity">
    ...
    <div class="content">
      <!--提交回访记录-->
      <div class="form-box" v-if="visitInfo&&visitInfo.name">
        <p class="title">当前回访阶段：{{visitInfo.name}}</p>
        <div class="text-box">
          <el-input
            :rows="4"
            type="textarea"
            placeholder="请输入回访内容"
            v-model="visitInfo.value">
          </el-input>
          <div class="button-box">
            <!--4-暂存；2-提交；3-回捞-->
            <el-button @click="toSave(4)">暂存</el-button>
            <el-button type="primary" v-if="visitInfo.type==1" @click="toSave(2)">提交</el-button>
            <el-button type="primary" v-if="visitInfo.type==2" @click="toSave(2)">确定丢单</el-button>
            <el-button type="primary" v-if="visitInfo.type==2" @click="toSave(3)">回捞</el-button>
          </div>
        </div>
      </div>
      <!--过去记录列表-->
