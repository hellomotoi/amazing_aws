        ...
        <div class="step-title-box">
          <ul class="step-title">
            <li class="step-item"
              ...
              <span v-if="item.completion!='past'"
                    @click="getStepList(item)">{{item.name}}</span>
              <el-tooltip
  ...
  export default{
    ...
    computed  : {
      ...
      canEdit(){
         ...
         * future   未进行
         */
        if(this.currentNode.completion != 'present' || this.target != 'followUp'){
          return false;
        }
        return true;
      }
    ...
    methods   : {
      ...
      /*标记主节点为完成*/
      submit(){
        if (this.currentNode.completion != 'present' || this.target != 'followUp') return;
        let status = '';
        ...
        adviserNodeSubmit(data).then(res => {
          if (res.data.code != 200) {
            ...
            if (this.currentNode.type == 'toBefiled') {
              this.$message({
                message    : '恭喜你，所有任务已完成！',
                showClose  : true,
                customClass: "succ",
