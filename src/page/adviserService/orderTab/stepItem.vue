      ...
      <div class="content"
        ...
        <dl v-for="item in listData">
          ...
          <dd v-if="!itemData.boxnum" class="alone">
            ...
            <!--type:4 签署协议-->
            <p v-if="item.type==4" class="agreement-box">
              <!--未完成-->
              <el-button
                v-if="item.status=='no'"
                @click="agreementHandle(item)">编辑
              </el-button>
              <!--完成-->
              <el-button class="disabled"
                v-if="item.status=='yes'">编辑
              </el-button>
              <!--完成：协议-->
              <a class="item-name"
                 v-if="item.status=='yes' && item.value"
                 :href="item.url" target="_blank">
                <i class="el-icon-document"></i>{{item.value}}
              </a>
              <!--完成：不需要签署-->
              <span style="color: #919191"
                    v-if="item.status=='yes' && item.text">{{item.text}}</span>
            </p>
            <!--type:5 老数据，附件只读-->
            <p v-if="item.type==5" class="readonly-box">
              <button disabled="disabled" type="button" class="el-button el-button--primary is-disabled">
                <span><i class="upload-icon"></i> <span>上传文件</span></span>
              </button>
...
<script>
  ...
  export default{
    ...
    methods   : {
      ...
      /*签署协议*/
      agreementHandle(item){
        console.log(item)
        if (item.agreementStatus == 1) return;
