<!--转单-->
<template>
  <el-dialog title="分配" top="10%" v-model="dialogVisible" id='adviserAgain' :show-close='false'
             :close-on-click-modal="false">
    <ul class="adviserAgain">
...
<script type="text/javascript">
  ...
  import {getAssignmentAdviserList, AssignmentAgainSubmit} from '../../service/getData.js';
  ...
  export default {
    ...
    created() {
      ...
      Bus.$off('openAdviserAgain');
      Bus.$on('openAdviserAgain', data => {
        console.log('传入数据', data);
        this.idArr = data.ids.toString();
        this.historyAdviserId = data.historyPlanner;
        console.log(this.historyAdviserId);
    ...
    methods: {
      ...
      submit() {
        AssignmentAgainSubmit({'uid': this.currentUserId, 'ids': this.idArr, 'plannerId': this.currentAdviserId})
          .then(res => {
      ...
      renderAdviserList() {
        getAssignmentAdviserList({'page': this.currentPage, 'plannerId': this.historyAdviserId})
          .then(res => {
...
<style lang="scss">
  ...
  #adviserAgain .el-dialog__header {
    padding: 30px 20px;
    span {
      font-size: 18px;
      color: #343434;
  ...
  #adviserAgain .dialog-footer button {
    width: 109px;
    line-height: 12px;
  }
  #adviserAgain .el-button + .el-button {
    margin-left: 18px;
  }
  /* pagination 分页 */
  #adviserAgain .pagination-box {
    position: absolute;
    top: 24px;
    right: 70px;
    .pagination-pageSize {
